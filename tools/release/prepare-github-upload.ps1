...
Write-BashScript -Path (Join-Path $uploadDir 'publish-step1-cdn.sh') -Lines @(
    ...
    ''
    '# A brand new .git while the repository already has commits on GitHub: adopt the published'
    '# history first, otherwise the push is rejected as unrelated and only a force push would get the'
    '# release out. The filtered fetch takes the commits and trees without the file contents, so it'
    '# costs a few kilobytes instead of the whole payload. --soft touches neither the index nor the'
    '# files, so the next commit holds exactly this build - including the removal of everything that'
    '# disappeared from app/.'
    'if ! git rev-parse --verify HEAD >/dev/null 2>&1 \'
    '        && git ls-remote --exit-code --heads origin main >/dev/null 2>&1; then'
    '    echo "== adopting the published history of $REPO"'
    '    git fetch --no-tags --filter=blob:none origin main || git fetch --no-tags origin main'
